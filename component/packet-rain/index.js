...
const sendScore = require('../../utils/config.js').httpConfig.sendScore;
var shapeArray = []
Component({
  properties: {
     // 是否开始展示游戏
  ...
  methods: {
        ...
        success(data) {
          if (data.data.flag) {
            ...
            that.setData({
              showStatus: 3,
              // isGameOver: true,
              couponname: data.data.data.name,
              introtext: data.data.data.intro
            })
          } else {
            wx.hideLoading()//关闭loding
            wx.showLoading({//开始加载loding
    ...
    audioOfClick() {
      ...
      innerAudioContext.src = 'https://sourced.sgsugou.com/rain/dianji.mp3'
